...
save(urls, file = "Data/urls_overview_pages.Rdata")
```
...
```{r}
load("Data/urls_overview_pages.Rdata")
...
save(process, urls, file = "Data/general_bills_information.Rdata")

tmp <- process %>% 
filter(grepl(url, pattern = "WP14/")) %>%
filter(drucksache_numm %>% grepl(pattern = "BT(.*)\\(Gesetzentwurf\\)")) %>% 
filter(!duplicated(drucksache_numm))

urls <- tmp$drucksache_numm %>% str_extract("BT(.*)\\(Gesetzentwurf\\)") %>% get_links
...
download.file(urls[jj], destfile = paste0("Data/Proposal/", filenames[jj], ".pdf"), quiet = T)
setTxtProgressBar(pb, jj)
}
...
lp <- tmp$url %>% str_extract("/WP\\d*/") %>% str_extract("[[:digit:]]+")
